...
class MongoDBTypeMapper:
    ...
    @staticmethod
    def flatten_document(doc: dict, prefix: str = "", max_depth: int = 3) -> dict[str, Any]:
...
class MongoDBSourceConnector(BaseSourceConnector):
    ...
    def _change_to_event(self, change: dict) -> Optional[ChangeEvent]:
    ...
    def _infer_columns_from_documents(self, documents: list[dict]) -> list[ColumnDefinition]:
