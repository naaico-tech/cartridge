...
@click.option(
    "--schema",
    help="Schema name for single mode (overrides config)"
)
@click.option(
    "--dry-run",
    is_flag=True,
...
@click.option(
    "--full-resync",
    is_flag=True,
    help="Perform full resync ignoring existing markers"
)
def run(
    config: Path,
    mode: Optional[str],
...
"""
    ...
    console.print("[blue]Edit the file with your database connections and schema settings.[/blue]")
...
def _display_config_summary(config: WarpConfig):
    ...
    table.add_row("Prometheus", "Enabled" if config.monitoring.prometheus.enabled else "Disabled")
