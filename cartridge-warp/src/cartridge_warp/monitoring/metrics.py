...
class MetricsCollector:
    ...
    def increment_records_processed(
        self, schema_name: str, table_name: str, operation: str
    ):
    ...
    def increment_error_count(self, schema_name: str, table_name: str, error_type: str):
        """Increment the error count counter."""
        self.error_count_total.labels(
            database="", schema=schema_name, table=table_name, error_type=error_type
        ).inc()

    def increment_schema_changes(
        self, schema_name: str, table_name: str, change_type: str
    ):
        """Increment the schema changes counter."""
        self.schema_changes_total.labels(
            database="", schema=schema_name, table=table_name, change_type=change_type
        ).inc()
    ...
    def record_schema_status(self, schema_name: str, status: str):
        ...
        self.sync_status.labels(database="", schema=schema_name, table="").set(
            status_value
        )

    def record_table_sync(
        self, schema_name: str, table_name: str, status: str, count: int
    ):
        """Record table sync operation."""
        status_value = {"success": 1, "error": -1}.get(status, 0)
        self.sync_status.labels(database="", schema=schema_name, table=table_name).set(
            status_value
        )
